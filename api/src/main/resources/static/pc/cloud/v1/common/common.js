...
function addExternalTab(title, url) {  
 ...
 function myformatterAll(date){
	 ...
	 * @param fn
	 */
	function notSupportOperation(opt,message,fn){
		if("DBL" == opt){
			if(message =='' || message==null){
				promptMessageCallBack("1","不支持此操作",fn);
			}else{
				promptMessageCallBack("1",message,fn);
			}
		}

		if("NOAUTH" == opt){
			if(message =='' || message==null){
				promptMessageCallBack("1","抱歉，您暂无此权限",fn);
			}else{
				promptMessageCallBack("1",message,fn);
			}
		}
