...
function zjSdlistquery(){   
	$('#zjSdLiDg').datagrid({ 
		pagination:true,
		method:"get",
	    url:ctx+'zj/zjs' , 
        queryParams:   ($("#zjSdlistFM").serialize()),
	    remoteSort: false, 
        columns: [[
...
function zjReloadgrid() {  
	$('#zjSdLiDg').datagrid('loadData',{total:0,rows:[]}); //清空DataGrid行数据
	$('#zjSdLiDg').datagrid('options').queryParams= serializeFormToJSON($("#zjSdlistFM").serializeArray());  
	$("#zjSdLiDg").datagrid('reload');
}  



function openApplySd(){
	var row = getSingleTreeGridSelectData($("#zjSdLiDg"));
	if(row !=null){
		$('#_loadDialog_zjSdlist').dialog({    
		    title: '人工屏蔽专家',    
		    width: '99%',    
		    height: 500,    
		    closed: false,    
		    cache: false,    
		    maximizable:true,
		    href: ctx+'zj/out/'+row.id+"/shield",    
		    modal: true   
		}); 
	}
}

function openApplyUnSd(){
	var row = getSingleTreeGridSelectData($("#zjSdLiDg"));
	if(row !=null){
		$('#_loadDialog_zjSdlist').dialog({    
		    title: '解除屏蔽专家',    
		    ...
		    cache: false,    
		    maximizable:true,
		    href: ctx+'zj/out/'+row.id+"/unshield",    
		    modal: true   
		}); 
	}
}
