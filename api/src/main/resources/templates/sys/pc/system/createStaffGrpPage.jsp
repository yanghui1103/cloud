...
$("button",navTab.getCurrentPanel()).click(function(){
	dwzConfirmFormToBack("是否确认新建用户组?",function(){
...
<body>
	<div class="pageContent">
		<form id="grpFm"   method=post 
			action="<%=basePath %>system/createStaffGrp?navTabId=page102&callbackType=closeCurrent"
			class="pageForm required-validate"
			onsubmit="return validateCallback(this,navTabAjaxDone);">
			<div class="pageFormContent" layoutH="56">
				<p>
					<label>组名称：</label> <input name="group_name" class="required" minlength="2"  
						type="text"   size="30" maxlength=30 />
				</p>
				<p>
					<label>用户：</label><input type="text" class="required" style="float: left" readonly lookupGroup="orgLookup1" name="orgLookup1.names"   />
					<input type="hidden" id="topIds" lookupGroup="orgLookup1"  name="orgLookup1.ids"   />
					<input name="temp_str1" id="temp_str1" type="hidden">
					<a href='<%=basePath %>system/openSysAddressBook/0/12222/1/${uuid}/1' target="dialog"
